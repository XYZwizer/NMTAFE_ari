...
class ChatModel:
	def __init__():
		...
		model = AutoModelForCausalLM.from_pretrained(checkpoint,device_map="cpu")  # You may want to use bfloat16 and/or move to GPU here
		self.chat = Chat()
...
"""},
        ...
        self.tokenized_chat = tokenizer.apply_chat_template(templated_chat, tokenize=True, add_generation_prompt=True,return_tensors="pt")
        self.run_over = False
    ...
    def doNextPromt(self,promnt):
        ...
        self.tokenized_chat = torch.cat((self.tokenized_chat,new_tokenized_message['input_ids']),dim=1)
        print("prompting")
        Stime = time.time()
        old_len = len(self.tokenized_chat[0])
        self.tokenized_chat = model.generate(self.tokenized_chat, max_new_tokens=64, temperature=0.1)
