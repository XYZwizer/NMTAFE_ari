...
"""},
        ...
        self.tokenized_chat = self.tokenizer.apply_chat_template(
            templated_chat, tokenize=True, add_generation_prompt=True, return_tensors="pt").to(self.model.device)
        self.run_over = False
        self.doNextPromt(spawnMessage)

    def doNextPromt(self, promnt):
        new_tokenized_message = self.tokenizer(f"<|start_header_id|>user<|end_header_id|>{promnt}<|eot_id|>", return_tensors="pt").to(self.model.device)
